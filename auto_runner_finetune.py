...
BASE_CONFIG = {
    ...
    'npoint': 16384,  
    'batch_size': 4,
    'conf_channel': False,  # 是否使用confidence channel

    # 测试参数
    'test_npoint': None,   # 测试时的点数，None 则使用 npoint 的值
    'test_max_point': 65536,  # 测试时的最大点数限制，避免OOM
    'test_batch_size': 1,
    'threshold': 0.5,
    'num_votes': 10,       # 测试时的投票数
    'test_ckpt': 'model.pth',
    'skip_test': False,    # 如果为 True，则只训练不测试
}
...
EXPERIMENT_LIST = [    
    {
        'name': 'onlyFT_Exp6_1_NLL_weight[None]',
        'loss_type': 'nll',
        'nll_weight': None,
    },
    {
        'name': 'onlyFT_Exp6_3_FL_weight[None]',
        'loss_type': 'focal',
        'focal_weight': None,
    },
    {
        'name': 'onlyFT_Exp6_5_FAFL',
    ...
    try:
        ...
        if current_epoch >= target_epoch:
            ...
            if current_epoch > 0:
                ...
                # 如果最新的checkpoint是model.pth，将其重命名为ckpt_<epoch>.pth
                if os.path.basename(latest_checkpoint) == 'model.pth':
                    new_checkpoint_name = f'ckpt_{current_epoch}.pth'
                    new_checkpoint_path = os.path.join(checkpoints_dir, new_checkpoint_name)
                    
                    # 重命名文件
                    import shutil
                    shutil.move(latest_checkpoint, new_checkpoint_path)
                    write_summary(f"    已将 model.pth 重命名为 {new_checkpoint_name}")
                    
                    checkpoint_name = new_checkpoint_name
                    latest_checkpoint = new_checkpoint_path
                else:
            ...
            # 添加conf_channel参数
            if config.get('conf_channel', False):  # 默认为False
                train_cmd.append('--conf_channel')
