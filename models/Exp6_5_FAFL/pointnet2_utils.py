...
def pc_normalize(pc):
    ...
    pc = pc - centroid
    m = np.max(np.sqrt(np.sum(pc**2, axis=1)))  #  计算每个点到原点的实际距离（平方后开根），然后np.max 找到所有点中到原点的最大距离 m
    pc = pc / m # 归一化
    return pc

# 确定每个点到采样点的距离，用于ball_query过程
    # 欧式距离
    # 函数输入是两组点，N为第一组点src个数，M为第二组点dst个数，C为输入点的通道数（如果xy时C=2）
...
def index_points(points, idx):
    ...
    """
    # idx为每各样本中所要选取的点的索引
    # 这里输入的点云数据B*8192*3，其中B为Batch_size，样本数  8192为一个文件中的光子数
    # 简单来说，这个函数就是要在点云数据中选取每个样本中索引值在idx这个索引数组里面的点
    # idx的长度为4时，则最后输出的为B*4*3，也就是在8192个点中选取在索引值idx的点
...
def farthest_point_sample(xyz, npoint):
    ...
    for i in range(npoint): # 更新第i个最远点
        ...
        centroid = xyz[batch_indices, farthest, :].view(B, 1, 2)#得到当前采样点的坐标 B*2!(改为2)  # 计算点集中的所有点到这个最远点的欧式距离
...
# 寻找球形邻域中的点
    ...
    # new_xyz为centroids点的数据,为S个球形领域的中心（由最远点采样在前面得出）
    # xyz为所有的点云数据
...
def query_ball_point(radius, nsample, xyz, new_xyz):
    ...
    group_idx[sqrdists > radius ** 2] = N # 找到距离大于给定半径的设置成一个N值（1024）索引 
                                          # 做升序排列，前面大于radius^2的都是N（最大值）所以函数将直接在剩下的点中取出前nsample个点
    group_idx = group_idx.sort(dim=-1)[0][:, :, :nsample] # 做升序排序，就选nsample个，后面的都是大的值（1024）
...
# Sampling + Grouping 主要用于将整个点云分散成局部的group
    # 对于每一个group都可以用PointNet单独的提取局部的全局特征
    # Sampling + Grouping 分成了sampl_and_group和sampl_and_group_all两个函数
    ...
    # 相当于先 最远点采样获得new_xyz
    #      后 将每个new_xyz当作一个group的中心点， 再获取这些点的球形区域内的点
...
# 将整个点云分散成一个group，用PointNet提取全局特征
...
# PointNetSetAbstraction类实现普通的SetAbstraction：
...
class PointNetSetAbstraction(nn.Module):
    ...
    # 128=npoint:最远点采样获得的点数
    # 0.4=radius:局部邻域的搜索半径
    # 64=nsample:局部邻域内的采样点数
    # [128,128,256]:每个点MLP输出的大小
    ...
    def forward(self, xyz, points):
        ...
        # 对每个group做一个max pooling得到局部的全局特征,得到的new_points:[B,3+D,npoint]
            # 3+D是坐标+局部全局特征（应该是局部邻域内的D个点的特征？）， npoints是最远点采样获得的所有采样点
...
#PointNetSetAbstractionMsg类实现MSG方法的Set Abstraction:
    # MSG实现：
        #radius_list输入的是一个list，例如[0.1，0.2,0.4]
...
class PointNetSetAbstractionMsg(nn.Module):
    def __init__(self, npoint, radius_list, nsample_list, in_channel, mlp_list):
        ...
        for i in range(len(mlp_list)):
            convs = nn.ModuleList()
            bns = nn.ModuleList()
            last_channel = in_channel + 2  # 这里修改为2，应该是多加的法向量通道也只有二维
    ...
    def forward(self, xyz, points):
        ...
        Return:
            ...
            new_points_concat: sample points feature data, [B, D', S]
        """
        xyz = xyz.permute(0, 2, 1) # 坐标点位置特征
        if points is not None:
            points = points.permute(0, 2, 1) # 额外提取的特征，第一次的时候就是那个法向量特征
        ...
        for i, radius in enumerate(self.radius_list):
            ...
            grouped_xyz -= new_xyz.view(B, S, 1, C) # 去mean new_xyz相当于簇的中心点，相当于去除不同中心点坐标的影响
            if points is not None:
...
class PointNetFeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        if S == 1:
            ...
            # 当点的个数大于一个的时候，采用线性差值的方式进行上采样
            dists = square_distance(xyz1, xyz2) # 确定每个点到每个采样点的距离
